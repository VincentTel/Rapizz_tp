...
/*
	public void UpdateCommande(Commande c)
	{
		try
		{
			String queryPrepared = "UPDATE commande set PrixTotal = ?, DateCommande = ? , DateLivraison = ?, ID_Livreur = ?, ID_Client = ?, Immat = ? where ID = ?";
			PreparedStatement pr_stmt = connection
					.prepareStatement(queryPrepared);
			pr_stmt.setFloat(1, c.getPrixTotal());
			pr_stmt.setDate(2, c.getDateCommande());
...
*/
	public List<Commande> getAllCommandeManage()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				Commande c = new Commande(rs.getLong(1), rs.getFloat(2),rs.getDate(3), rs.getDate(4));
				result.add(c);
			}
		}
		catch (Exception err)
